...
// Save guess data with wallet-specific key
const walletAddress = registeredWallet.toLowerCase();
const walletGuessKey = `guesses_${walletAddress}`;

// Get existing guesses for this wallet
const existingGuessesRaw = localStorage.getItem(walletGuessKey);
...
localStorage.setItem(walletGuessKey, JSON.stringify(existingGuesses));
localStorage.setItem(`lastGuess_${walletAddress}`, JSON.stringify(exportableData));

// Also save to individual guess key for backward compatibility
localStorage.setItem(`guesses/${walletAddress}/${guessId}`, JSON.stringify(exportableData));
