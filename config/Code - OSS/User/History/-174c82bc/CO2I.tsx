...
// WALLET-SPECIFIC HELPER FUNCTIONS (FIXED)
...
const getWalletLocalStorage = (walletAddress: string, key: string): any => {
  try {
    ...
    console.error(`Error reading wallet storage for key "${key}":`, error);
    return null;
  }
};
...
const setWalletLocalStorage = (walletAddress: string, key: string, value: any): void => {
  try {
    ...
    console.error(`Error writing wallet storage for key "${key}":`, error);
  }
};

// MIGRATION FUNCTION: Migrate old data to wallet-specific format (FIXED)
const migrateOldDataToWalletSpecific = (walletAddress: string): void => {
  try {
    ...
    if (oldAllGuesses && !alreadyMigrated && oldAllGuesses !== "undefined" && oldAllGuesses !== "null") {
      ...
      try {
        ...
        console.error("Error parsing old guess data:", parseError);
      }
    }
  } catch (error) {
...
const VerifyOffChain: React.FC = () => {
  ...
  useEffect(() => {
    const initWeb3 = async () => {
      try {
        ...
        console.log("✅ Web3 initialized. Current block:", blockNumber);
      } catch (error) {
        console.error("Error initializing Web3:", error);
        setAlertMessage({
          type: "error",
          title: "Web3 Initialization Failed",
          message: "Unable to connect to Polygon Amoy network. Please check your connection.",
        });
      }
    };
  ...
  // UPDATED: Load guess data from wallet-specific localStorage (FIXED)
  useEffect(() => {
    const loadGuessData = async () => {
      ...
      try {
        ...
        // FIXED: Load from wallet-specific localStorage with proper error handling
        const allGuesses = getWalletLocalStorage(connectedAccount, "allGuessSubmissions");
        
        if (!allGuesses || !Array.isArray(allGuesses)) {
          console.warn("No guess submissions found in wallet-specific storage");
          setAlertMessage({
            type: "warning",
            title: "No Guess Data",
        ...
        if (currentGuess) {
          ...
          console.log("✅ Guess data loaded:", {
            guessId,
            targetBlock,
            tokenSize: currentGuess.tokenSize,
            complex: currentGuess.complex
          });
        } else {
        ...
        setAlertMessage({
          ...
          message: "Failed to load guess data. Please try again.",
        });
        setIsLoading(false);
      }
    };
  ...
  const fetchTargetBlockHash = async () => {
    if (!web3Instance) {
      setAlertMessage({
        ...
        message: "Please wait for Web3 to initialize and try again.",
      });
      return;
    }

    if (!targetBlockCount || targetBlockCount === 0) {
      setAlertMessage({
        type: "error",
        title: "Invalid Block Number",
        message: "Target block number is not set. Please submit a guess first.",
      });
      return;
    }
    ...
    if (!actualHash || actualHash === "") {
      setAlertMessage({
        type: "error",
        title: "Missing Hash Data",
        message: "Actual hash is not available. Please submit a guess first.",
      });
      return;
    }
    ...
    try {
      console.log("🔍 Fetching target block:", targetBlockCount);
      ...
      console.log("📊 Current block:", currentBlock, "| Distance:", distanceToTarget);
      ...
      if (complex && distanceToTarget > 256) {
        ...
        console.log("🎲 Complex calc:", {
          targetBlock: targetBlockCount,
          byteHex,
          byteValue,
          adjustedPosition,
          randomBlock: randomBlockNumber
        });
        ...
        console.log("📦 Fetching simple block hash...");
        const block = await web3Instance.eth.getBlock(targetBlockCount);
        if (!block || !block.hash) {
          throw new Error("Block not found or not yet mined. Please wait for the block to be confirmed.");
        }
        fetchedHash = block.hash.toString();
      }

      console.log("✅ Fetched hash:", fetchedHash.substring(0, 20) + "...");
      ...
      console.log("🎯 Comparing tokens...", {
        guessTokenCount: guessTokens.length,
        fetchedTokenCount: fetchedTokens.length
      });
      ...
      if (matches.length > 0) {
        ...
        // FIXED: Save to wallet-specific localStorage with proper error handling
        if (connectedAccount) {
          ...
          console.log("💾 Match data saved to wallet-specific storage");
        }

        setAlertMessage({
          type: "success",
          title: "Match Found!",
          message: `Found ${matches.length} matching token(s)!\n\nMatches: ${matches.join(", ")}\n\nClick "Proceed to On-Chain Verification" to continue.`,
        });
      } else {
        setAlertMessage({
          type: "warning",
          title: "No Matches",
          message:
            "No matching tokens found between your guess and the blockchain hash. You may need to wait or try a different guess.",
        });
      }
    } catch (error: any) {
      console.error("❌ Error fetching block hash:", error);
      
      let errorMessage = "Failed to fetch block hash. ";
      
      if (error.message?.includes("not found") || error.message?.includes("not yet mined")) {
        errorMessage += "The target block may not be mined yet. Please wait and try again.";
  ...
  const handleProceedToOnChain = () => {
    ...
    // FIXED: Get matchData from wallet-specific localStorage with error handling
    ...
    console.log("✅ Proceeding to on-chain verification with:", {
      guessId,
      matches: matchData.matchedTokens.length,
      targetBlock: matchData.targetBlockNumber
    });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            ...
            <button
              ...
              disabled={isFetching || !targetBlockCount || !web3Instance}
              className="w-full mt-6 py-3 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 disabled:from-gray-600 disabled:to-gray-700 disabled:cursor-not-allowed text-white rounded-lg font-semibold transition-all flex items-center justify-center gap-2"
            >
              {isFetching ? (
                <>
                  <motion.div
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {foundTokens.map((token, idx) => {
                      const isMatch = actualHash && tokenSize > 0 && tokenize(actualHash, tokenSize).includes(token);
                      return (
                        <motion.div
                          key={idx}
                          initial={{ opacity: 0, scale: 0.8 }}
