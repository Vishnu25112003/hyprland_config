    ...
    const loadGuessData = () => {
      ...
      if (!account) {
        showAlert(
          ...
          "Please connect your wallet to verify a guess.",
          () => navigate("/dashboard"),
        );
        return;
      }
      ...
      if (data) {
        ...
        // Generate actual tokens from user's hash
        const tokens = tokenize(removePrefix(data.actualHash), data.tokenSize);
        setActualTokens(tokens);
      } else {
        showAlert(
          "error",
          "Data Not Found",
